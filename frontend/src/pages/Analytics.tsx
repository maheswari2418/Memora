...
const PIE_COLORS = ['#7C3AED', '#58A6FF', '#3FB950', '#D29922', '#F85149', '#8B949E'];
    ...
    <div style={{ padding: 24, background: '#0D1117', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          ...
          { label: 'Auto-Resolved', value: '68%', sub: 'was 12%', color: '#7C3AED' },
          { label: 'Memory Nodes', value: a.totalMemoryNodes.toLocaleString(), sub: `${a.totalMemoryEdges} edges`, color: '#D29922' },
        ].map(kpi => (
          <div key={kpi.label} style={{ background: '#161B22', border: '1px solid #30363D', borderRadius: 8, padding: '16px 18px' }}>
            <div style={{ fontSize: 10, color: '#8B949E', fontFamily: 'JetBrains Mono, monospace', marginBottom: 8 }}>{kpi.label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        {/* MTTR Trend */}
        <div style={{ background: '#161B22', border: '1px solid #30363D', borderRadius: 8, padding: '16px 18px' }}>
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 14 }}>MTTR Trend (minutes) <span style={{ color: '#3FB950', fontFamily: 'JetBrains Mono, monospace', fontSize: 10 }}>↓ 90%</span></div>
        ...
        {/* Recall Accuracy */}
        <div style={{ background: '#161B22', border: '1px solid #30363D', borderRadius: 8, padding: '16px 18px' }}>
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 14 }}>Recall Accuracy (%) <span style={{ color: '#58A6FF', fontFamily: 'JetBrains Mono, monospace', fontSize: 10 }}>+24pp</span></div>
          <ResponsiveContainer width="100%" height={180}>
            ...
            <div style={{ flex: 1 }}>
              {a.rootCauseBreakdown.map((d, i) => (
                <div key={d.category} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: '#E6EDF3', fontFamily: 'JetBrains Mono, monospace' }}>{d.count}</span>
                </div>
              ))}
            </div>
          </div>
